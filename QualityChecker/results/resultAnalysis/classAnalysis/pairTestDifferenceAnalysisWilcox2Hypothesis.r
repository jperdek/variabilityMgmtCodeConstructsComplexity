...
wilcoxTest <- wilcox.test(firstFormData[["Cycl..Number"]], secondFormData[["Cycl..Number"]], 
                          alternative=chosenAlternative, paired=ifPaired, exact = TRUE, correct = TRUE,
                          conf.int = TRUE, conf.level = 0.95)
print(wilcoxTest)
print(finalWilcoxTest)

write.table(finalWilcoxTest, ".\\pairedClassTestForm1-4.csv", row.names=FALSE, dec=",", sep=";")
